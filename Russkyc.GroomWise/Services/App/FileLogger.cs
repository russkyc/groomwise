...
public class FileLogger : ILogger
{
    ...
    public FileLogger(IConfigurationService configurationService)
    {
        ...
        if (_isLoggingEnabled) Log(this,"Starting application environment");

    }
    public void Log(string message)
    {
        if (_isLoggingEnabled) _path.StreamAppend($"{Environment.NewLine}[{DateTime.Now:yy-MM-dd HH:mm:ss}]{message}");
    }

    public void Log(object sender, string message)
    {
        if (_isLoggingEnabled) _path.StreamAppend($"{Environment.NewLine}[{DateTime.Now:yy-MM-dd HH:mm:ss}]{sender.GetType()}: {message}");
    }
}