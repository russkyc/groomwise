...
public class EncryptedRepository<T> : Interfaces.Repository.IRepository<T>
    where T : class, new()
{
    ...
    )
    {
        ...
        _collection = Task.Run(async () =>
        {
            var entities = await databaseService.GetCollection<T>();
            return entities.Select(encryptionService.Decrypt);
        })
            .Result.ToList();
    }
